...
- Combat Intrigue cards: Contingency Plan uses a typed `combat-intrigue` fixed strength spec; Backed by CHOAM uses a typed `combat-intrigue` completed-contract-gated strength spec; Weirding Combat uses typed `combat-intrigue` base and Bene Gesserit Influence-threshold strength specs.
