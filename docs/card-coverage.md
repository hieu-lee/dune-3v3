...
- Imperium cards: Bene Gesserit Operative, Captured Mentat, Cargo Runner, Chani, Clever Tactician, Covert Operation, Dangerous Rhetoric, Double Agent, Ecological Testing Station, Fedaykin Stilltent, Guild Envoy, Hidden Missive, Imperial Spymaster, Maker Keeper, Maula Pistol, Northern Watermaster, Paracompass, Price is No Object, Rebel Supplier, Reliable Informant, Sardaukar Soldier, Southern Elders, Space-time Folding, Stilgar, The Devoted, Wheels Within Wheels.
...
- Imported Imperium Agent text that is only summarized in `play` remains manual until migrated into Agent effect specs; fixed Intrigue draws, fixed resource gains, and fixed routed troop recruitment are covered for the migrated simple cards, while cards with costs beyond current payment shapes, discard/trash choices, Influence gains, contract acquisition, spy placement, acquire bonuses beyond fixed VP/resource/spy/Intrigue rewards, Recall Agent, sandworm effects, or opponent effects beyond typed hand discard still need specific primitives before automatic generation.
