...
- Corrinth City uses a typed Agent discard-for-reward effect for discarding two cards and spending 5 Solari to gain 1 VP, plus a typed default +5 persuasion Reveal branch. Delivery Agreement uses the same Agent primitive to discard one card before taking a face-up CHOAM contract, plus a typed default Reveal spice reward. Corrinth City's paid High Council Reveal branch and Delivery Agreement's completed-contract trash-for-VP Reveal branch remain explicit future choice primitives; neither card grants VP on acquisition.
