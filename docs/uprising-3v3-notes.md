...
- Current Imperium-card implementation tracks Maker board-space Agent visits through the round so Smuggler's Harvester can pay its conditional Reveal spice, and resolves Interstellar Trade's Reveal persuasion from the number of completed contracts when the Reveal plan is created.
