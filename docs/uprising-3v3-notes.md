...
- Current Imperium-card implementation tracks Maker board-space Agent visits through the round so Smuggler's Harvester can pay its conditional Reveal spice, resolves Interstellar Trade's Reveal persuasion from completed contracts when the Reveal plan is created, and prompts Calculus of Power's optional Reveal trash for another Emperor card in play before adding its 3 strength.
