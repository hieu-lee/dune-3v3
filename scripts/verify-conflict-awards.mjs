...
try {
  ...
  const shadowContestResult = state.startNextRound(shadowContestReward);
  const shadowContestWinner = playerById(shadowContestResult, "p2");
  assert.equal(
    shadowContestWinner.influence.bene,
    2,
    "Shadow Contest should pay first-place Bene Gesserit Influence",
  );
  assert.equal(shadowContestWinner.vp, 1, "Shadow Contest Influence should score the 2-Influence VP threshold");
  assert.equal(shadowContestWinner.intrigues.length, 1, "Shadow Contest should draw one first-place Intrigue");
  assert.equal(
    shadowContestResult.intrigueDeck.length,
    shadowContestReward.intrigueDeck.length - 1,
    "Shadow Contest should draw from the Intrigue deck",
  );
  assert.ok(
    shadowContestResult.log.some((entry) =>
      entry.includes("draws an Intrigue card") &&
      entry.includes("Shadow Contest")
    ),
    "Shadow Contest should log the Intrigue draw",
  );
  assert.ok(
    shadowContestResult.log.some((entry) =>
    ...
    "Shadow Contest should log the Influence reward",
  );

  const doubledShadowContestReward = fixture(state, data, (players) =>
    players.map((player) =>
      player.id === "p3"
        ? {
            ...player,
  ...
  const doubledShadowContestResult = state.startNextRound(doubledShadowContestReward);
  const doubledShadowContestWinner = playerById(doubledShadowContestResult, "p3");
  assert.equal(
    doubledShadowContestWinner.influence.bene,
    2,
    "Sandworms should double Shadow Contest Bene Gesserit Influence",
  );
  assert.equal(doubledShadowContestWinner.vp, 1, "Doubled Shadow Contest Influence should score the VP threshold once");
  assert.equal(doubledShadowContestWinner.intrigues.length, 2, "Sandworms should double Shadow Contest Intrigue draws");
  assert.equal(
    doubledShadowContestResult.intrigueDeck.length,
    doubledShadowContestReward.intrigueDeck.length - 2,
    "Doubled Shadow Contest should draw two cards from the Intrigue deck",
  );
  assert.ok(
    doubledShadowContestResult.log.some((entry) =>
      entry.includes("draws 2 Intrigue cards") &&
      entry.includes("Shadow Contest")
    ),
    "Doubled Shadow Contest should log both Intrigue draws",
  );
  assert.ok(
    doubledShadowContestResult.log.some((entry) =>
      entry.includes("gains 2 Bene Gesserit Influence") &&
      entry.includes("sandworm doubling")
    ),
    "Doubled Shadow Contest should log the doubled Influence reward",
  );
