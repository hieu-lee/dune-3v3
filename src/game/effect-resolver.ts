...
export type EffectResolverState = Pick<GameState, "roundMakerSpaceVisits" | "sharedSpyPosts" | "spyPosts">;
...
function conditionApplies(condition: GameEffectConditionSpec, context: GameEffectContext) {
  if (condition.kind === "visited-maker-space") {
    return context.state ? hasVisitedMakerSpaceThisRound(context.state, context.source.id) : false;
  }
  if (condition.kind === "has-spy-posts") {
    return context.state ? spyPostCount(context.state, context.source.id) >= condition.count : false;
  }
